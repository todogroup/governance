# 👩‍🏫 TODO Ambassador Program (Individuals)

TODO Ambassadors are official advocates within the TODO Group community who support the OSPO ecosystem.

## 🧩 What Ambassadors Do

TODO Ambassadors contribute in different ways depending on their experience and interests. You don't need to do everything!

Ambassadors are expected to:

* Make at least one contribution per month connected to a TODO initiative, resource, or community activity
* Advocate for OSPO best practices using and showcasing TODO Group resources
* Contribute to TODO resources, repositories, research, guides, and working-group deliverables
* Support, mentor, and onboard practitioners and newcomers
* Lead or support TODO Working Groups, OSPOlogy events, meetups, and other community initiatives
* Bring practitioner and community feedback back to TODO
* Represent TODO professionally, inclusively, and in line with the TODO Group mission

Examples of contributions include:

* Contributing to or reviewing the OSPO Book, TODO Guides, OSPO Glossary, Mind Map, Maturity Model, Awesome OSPO tool list, research studies, whitepapers, or other TODO resources
* Writing for the TODO blog or newsletter or mentioning TODO group in external blogs
* Contributing PRs, issues, reviews, translations, case studies, or other improvements to TODO repositories
* Co-chairing or moderating a TODO Working Group and helping move its deliverables forward
* Organizing or supporting OSPOlogy events / TODO community meetups in your region
* Creating educational content, talks, tutorials, or videos that demonstrate or reference TODO resources
* Answering community questions, mentoring practitioners, and helping newcomers contribute

General OSPO or open source content that is not connected to a TODO resource, initiative, or community activity does not normally count as an Ambassador contribution.
See the [TODO OSPO Ambassador Manual](./ambassador-manual.md) for detailed contribution guidance and examples.

## 🧩 Responsibilities

The TODO Ambassador Program encompasses a group of community leaders who provide support, training, mentorship, guidance, and rewards to:

* Engage and enable the TODO community through local meetups, OSPOlogy meetings, OSPO-focused content, and mentoring
* Foster strong community collaboration and relationships
* Attract and onboard new community participants
* Provide valuable input and feedback to the TODO community about community programs and initiatives
* Advocate OSPO best practices and TODO initiatives globally

## ✅ Requirements

The program requirements for becoming an official TODO OSPO Ambassador are:

* Must be 18 years of age or older
* Must follow the [TODO Code Of Conduct](https://todogroup.org/code-of-conduct/)
* Must comply with [LF antitrust policy](https://www.linuxfoundation.org/legal/antitrust-policy)

## 🔍 Evaluation Criteria

Applicants must meet at least one of the following criteria:

* Be an active contributor to a TODO initiative, such as Awesome OSPO, OSPO Landscape, TODO website or blog, TODO resources, research, or Working Group deliverables (meeting notes, summary blogs, etc)
* Be a representative from a TODO General Member organization that has contributed an organizational use case or other practical input from the organization to TODO Group resources or initiatives.
* Be an OSPOlogy Program Committee member, organizer, or roundtable mentor or speaker
* Be an active co-chair or facilitator in a TODO Working Group, with 1+ year of experience moderating, mentoring, or creating content related to the WG scope

Applicants should provide links or other reviewable evidence demonstrating their involvement with the TODO community as part of their application.

## 🚀 Benefits

TODO OSPO Ambassadors receive:

* An official profile on the TODO Ambassadors page
* A TODO OSPO Ambassador Credly badge issued by the Linux Foundation
* Recognition for their contributions to TODO and the OSPO community
* Access to a global network of fellow Ambassadors
* TODO Group and Linux Foundation platform support for events and local meetups
* Consideration for relevant opportunities across the Linux Foundation ecosystem
* Opportunities to contribute expertise to TODO-branded research, studies, whitepapers, events, and community initiatives
* Potential amplification of Ambassador contributions through TODO community channels
* Discount codes for selected Linux Foundation events
* Exclusive swag and giveaways, when available

The TODO OSPO Ambassador role is not a paid position.

## 🙋‍♀️ Frequently Asked Questions

### What is the process for becoming an Ambassador?

Review the [TODO OSPO Ambassador Manual](./ambassador-manual.md) and make sure you meet the current requirements.

If eligible, [submit an Ambassador application](https://github.com/todogroup/governance/issues/new/choose) with evidence of your involvement and contributions to the TODO community.

Applications are reviewed by the TODO Steering Committee.

### When can I apply?

You can apply at any time throughout the year, provided that you meet the application requirements.

### Do I need to organize an OSPO meetup?

No. Organizing meetups and OSPOlogy events remains an important contribution, but it is only one way to participate.

Ambassadors can also contribute through TODO resources and repositories, Working Groups, research, blogs and educational content, mentoring, community support, and other TODO initiatives.

### What counts as an Ambassador contribution?

A contribution should connect to a real TODO initiative or resource and help someone understand, use, improve, or contribute to it.

Examples include a merged PR to a TODO resource, contributing to a Working Group deliverable, moderating a Working Group, organizing an OSPOlogy event, publishing content that teaches practitioners how to use a TODO resource, or providing reviewable community support.

See the [TODO OSPO Ambassador Manual](./ambassador-manual.md) for the complete guidance.

### How are Ambassador contributions tracked?

Ambassadors submit their contributions using the Ambassador Contribution Submission form. Contributions should include a link or other reviewable evidence and identify the related TODO resource, initiative, or Working Group.

This helps TODO recognize Ambassador work and understand where Ambassadors are contributing across the community.

### Is the TODO OSPO Ambassador role a paid position?

No. The TODO OSPO Ambassador role is not a paid position. Ambassadors may receive recognition, selected Linux Foundation event discounts, swag, community opportunities, and other program benefits.

### How does changing organizations affect Ambassador status?

The Ambassador title is granted to the individual, not their employer. Changing organizations therefore does not automatically affect Ambassador status.

Ambassadors are still expected to maintain the participation and contribution requirements of the program.

### How long does my ambassadorship last?

Ambassadorships run for two years.

Ongoing participation is reviewed during the term and may consider consistency and quality of contributions, relevance to TODO initiatives, community impact, professionalism, responsiveness, and continued interest.

Sustained inactivity or lack of response to program communications may result in an earlier review.

At the end of the two-year term, Ambassadors may request renewal according to the current program requirements.

### Who do I contact with questions?

Questions about the Ambassador Program can be raised in the private TODO Ambassador Slack channel or sent to <steering-committee@todogroup.org>.

## 💚 Attribution

This Ambassador Program is adapted from the CNCF Ambassador Program, version 2.0, and AAIF Ambassador guidelines.
